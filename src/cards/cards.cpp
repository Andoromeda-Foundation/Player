	...
	void show() {
		...
		if (com.string.empty()) {
			for (int i=0;i<_.p2.size();++i) {
				if (_.p2[i].id == _.current_map_event_id) {
					com.string = lcf::DBString(_.p2[i].info());
					break;
				}
			}
		}
	...
	void atk() {
		...
		if (owner(this_id) == 1) {
			...
			if (i == _.p2.size()) {
				...
				auto this_monster = getMonster(this_id);
				_.p2[i].hp -= this_monster.AP;
				if (_.p2[i].hp <= 0) {
			...
			for (i=0;i<_.p1.size();++i) {
				that_event = Game_Map::GetEvent(_.p1[i].id);
				int xx = that_event->GetX(), yy = that_event->GetY();
				if (xx == x && yy == y + 1) {
					break;
				}
			...
			if (i == _.p1.size()) {
				...
				auto this_monster = getMonster(this_id);
				_.p1[i].hp -= this_monster.AP;
				if (_.p1[i].hp <= 0) {
					Main_Data::game_screen->ShowBattleAnimation(143, _.p1[i].id, 0);
					that_event->SetActive(false);
					_.p1.erase(_.p1.begin() + i);
				} else {
					Main_Data::game_screen->ShowBattleAnimation(142, _.p1[i].id, 0);
				}
			}
		}
	}
